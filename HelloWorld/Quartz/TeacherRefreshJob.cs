...
namespace StudyOnline.Quartz
{
    ...
    public class TeacherRefreshJob : IJob
    {
        ...
        {
            log.Info("now sync teacher online situations");
            ...
            {
                log.Debug("let the teacher offline :" + item.Username + " last Refresh=" + item.Refresh.Value.ToString("yyyy-MM-dd HH:mm:ss"));
